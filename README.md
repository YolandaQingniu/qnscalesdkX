...
* 在你的module的根目录下的**build.gradle**添加依赖
	```
	<!--这里的版本号，2.33.0-yf-opt 可以指定为任意release版本-->
	<!--如果希望一直使用最新版本可以替换 2.33.0-yf-opt master-SNAPSHOT -->
	dependencies {
	        ...
	        implementation 'com.github.YolandaQingniu:qnscalesdkX:2.33.0-yf-opt'
	}
	```
...
- 如果你的项目之前是使用本地集成的方式（即引入jar包与so库的形式），转换为在线gradle依赖后，务必在本地删除本地集成时引入的jar包与so库文件
